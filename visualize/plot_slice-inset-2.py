...
def plot_with_insets(slice_data, regions, region_colors, show_axis=True, show_scale_bar=True, show_color_bar=True,
                     ...
                     annotation_text='A', square_size=0.1, inset_width=0.2, inset_height=0.2, 
                     inset_spacing=0.1, inset_y_pos=0.14):
    ...
    # Add a white square with a capital letter in the top left corner
    square = patches.Rectangle((0.01, 0.89), square_size, square_size, transform=ax_main.transAxes,
                               edgecolor='black', facecolor='white', zorder=2)
    ax_main.add_patch(square)
    
    # Add centered text inside the square
    ax_main.text(0.01 + square_size / 2, 0.89 + square_size / 2, annotation_text,
                 transform=ax_main.transAxes, fontsize=24, color='black',
...
def plot_and_save_slice_with_insets(stack_path, slice_index, regions, region_colors, show_axis=True, show_scale_bar=True, show_color_bar=True,
    ...
    fig = plot_with_insets(slice_data, regions, region_colors, show_axis, show_scale_bar, show_color_bar,
                           ...
                           cbar_position, cbar_text_color, annotation_text, square_size, inset_width, inset_height, 
                           inset_spacing, inset_y_pos)
    save_plot(fig, output_path)

if __name__ == "__main__":
    stack_path = r"C:\Users\rausc\Documents\EMBL\data\droso-results\Drosophila20210316LogScale01L_Good_Sample_02_t_-average-100-offset--2.TIFF"
    slice_index = 100  # Specify the desired slice index here

    # Define regions as [y, x, height, width] for insets
    regions = [
        [65, 125, 40, 40],  # Region 1
        [100, 160, 40, 40],  # Region 2
        [100, 250, 40, 40]   # Region 3
    ]
    ...
    plot_and_save_slice_with_insets(stack_path, slice_index, regions, region_colors, show_axis=False, show_scale_bar=True, show_color_bar=True,
                                    ...
                                    scale_bar_position='lower right', scale_bar_font_size=12,
                                    cbar_position=[0.83, 0.7, 0.02, 0.12], cbar_text_color='white',
                                    annotation_text='E', square_size=0.1, inset_width=0.2, inset_height=0.2, 
                                    inset_spacing=0.1, inset_y_pos=0.14)
