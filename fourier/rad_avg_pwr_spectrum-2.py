...
def plot_radial_averages(all_radial_profiles, labels, output_dir):
    ...
    for radial_profiles, label in zip(all_radial_profiles, labels):
        ...
        # Normalize the mean radial profile to make it a probability density function
        sum_mean_radial = np.sum(mean_radial)
        norm_mean_radial = mean_radial / sum_mean_radial
        freq_range = np.arange(len(norm_mean_radial))
        plt.plot(freq_range, norm_mean_radial, label=label)

    plt.xlabel('Radius')
    plt.ylabel('Normalized Average Log Power Spectrum (PDF)')
    plt.title('Normalized Radial Averaged Power Spectrum (PDF)')
    plt.legend()
    plt.grid(True)

    plot_filename = 'normalized_radial_avg_power_spectrum_pdf-2.png'
    plt.savefig(os.path.join(output_dir, plot_filename), bbox_inches='tight')
    plt.close()
    print(f"Normalized radial averaged power spectrum plot saved to {os.path.join(output_dir, plot_filename)}")
