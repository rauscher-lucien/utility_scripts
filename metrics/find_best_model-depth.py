...
def aggregate_scores(psnr_files, ssim_files):
    ...
    for psnr_file, ssim_file in zip(psnr_files, ssim_files):
        ...
        for label, psnr_score, ssim_score in zip(psnr_labels, psnr_scores, ssim_scores):
            first_number = label.split()[0]
            aggregated_psnr_scores[first_number].append(psnr_score)
            aggregated_ssim_scores[first_number].append(ssim_score)
...
def calculate_overall_best(psnr_files, ssim_files):
    ...
    # Calculate mean scores for each first number
...
def plot_overall_scores(overall_scores, output_path):
    ...
    plt.xlabel('First Number in Labels')
    plt.title('Overall Scores for Different First Numbers in Labels')
    plt.grid(axis='y')
...
# Example usage
psnr_files = [
    r"C:\Users\rausc\Documents\EMBL\data\nema-results\psnr_scores-nema-2D-single-2.txt",
    r"C:\Users\rausc\Documents\EMBL\data\droso-results\psnr_scores-droso-2D-single-1.txt",
    r"C:\Users\rausc\Documents\EMBL\data\mouse-results\psnr_scores-mouse-2D-single-1.txt"
    # Add more PSNR file paths as needed
]
ssim_files = [
    r"C:\Users\rausc\Documents\EMBL\data\nema-results\ssim_scores-nema-2D-single-2.txt",
    r"C:\Users\rausc\Documents\EMBL\data\droso-results\ssim_scores-droso-2d-single-1.txt",
    r"C:\Users\rausc\Documents\EMBL\data\mouse-results\ssim_scores-mouse-2D-single-1.txt"
    # Add more SSIM file paths as needed
]

best_label, best_score, overall_scores = calculate_overall_best(psnr_files, ssim_files)
print(f"Best First Number: {best_label}")
print(f"Best Score: {best_score}")
print(f"Overall Scores: {overall_scores}")

# Plotting the results
output_path = r"C:\Users\rausc\Documents\EMBL\data\general-results\2D-single-overall-depth.png"
