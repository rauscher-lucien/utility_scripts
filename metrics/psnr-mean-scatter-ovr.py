...
def plot_psnr_scores_with_scatter_mean_std(all_psnr_scores, labels, output_dir, plot_filename, sample_colors, sample_names, dpi=100, font_size=12, line_thickness=1.5):
    ...
    for i, label in enumerate(labels):
        # Collect all PSNR scores for this label across all samples
        y_values = []
        for sample_idx, sample_name in enumerate(sample_names):
            scores = all_psnr_scores[(sample_name, label)]
    ...
    for x, y, color in all_points:
        plt.scatter(x, y, alpha=0.6, color=color, s=60, zorder=3)
    ...
    # Draw the grid lines behind the scatter plot points
    ...
    plt.close()
    print(f"PSNR scores scatter plot with mean and std saved to {plot_path}")
...
if __name__ == "__main__":
    ...
    plot_filename = 'psnr_scores-general-comp-1.png'
    ...
    plot_psnr_scores_with_scatter_mean_std(all_psnr_scores, custom_labels, output_dir, plot_filename, sample_colors, sample_names, dpi=dpi, font_size=font_size, line_thickness=line_thickness)
